...
app = Flask(__name__)
openai.api_key='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'
...
def index():
    if request.method == 'POST':
        ...
        # OCR the image
        text = pytesseract.image_to_string(img)#[:100]
        print(len(text),"<><><><><><><>")
        clauses = text.split("\n\n")
        list1 = []
        count = 1
        ...
        #print(FINAL_RESULT)
        #print('summary_output, type = >>>>>>>>',type(summary_output))
        #print(summary_output)
        #print('Summary_Analyse, type = >>>>>>>>',type(Summary_Analyse))
        #print(Summary_Analyse)
        #print('Summary_Text1, type = >>>>>>>>',type(Summary_Text1))
        #print(Summary_Text1)
        #print('zip_longest, type = >>>>>>>>',type(zip_longest))
        #print(zip_longest)
        list2 = ";".join(list2)
        FINAL_RESULT = ";".join(FINAL_RESULT)
        return jsonify(
        clauses = list2,
        indicator_per_clause  = FINAL_RESULT, 
        summary_output = summary_output,
        Summary_Analyse = Summary_Analyse,
        Summary_Text = Summary_Text1,
        )
